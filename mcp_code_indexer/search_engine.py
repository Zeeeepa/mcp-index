...
class SearchEngine:
    ...
    def natural_language_search(self, query: str, project_ids: Optional[List[str]] = None,
        ...
        """
        基于自然语言的代码搜索，使用RetrievalQA模式处理结果
        ...
        Returns:
            格式化后的搜索结果，包含LLM回答和相关代码片段
        """
        try:
            # 1. 获取原始搜索结果
            results = self.search(query, project_ids, filters, limit=limit)
            
            if not results:
                return {
                    "answer": f"未找到与'{query}'相关的代码。",
                    "context_files": [],
                    "search_query": query
                }
            ...
            # 3. 构建提示
            prompt_template = """使用以下代码上下文来回答问题。
如果你不知道答案，请说明你不知道，不要尝试编造答案。
在回答中引用相关代码片段，解释代码的功能和逻辑。
...
回答:"""
            ...
            # 这里可以使用OpenAI或其他LLM
            try:
                from mcp_code_indexer.llm_processor import LLMProcessor
                # 尝试获取LLM处理器
                llm_processor = getattr(self, 'llm_processor', None)
                
                if llm_processor:
                    # 直接使用已有的LLM处理器
                    messages = [
                        {"role": "system", "content": "你是一个代码助手，使用给定的代码上下文回答问题。"},
                        {"role": "user", "content": prompt_template.format(context=context_text, question=query)}
                    ]
                    
                    if hasattr(llm_processor, '_call_openai_api'):
                        answer = llm_processor._call_openai_api(messages)
                    elif hasattr(llm_processor, '_call_ollama'):
                        answer = llm_processor._call_ollama(messages)
                    else:
                        # 如果没有可用的LLM API，生成一个基本回答
                        answer = f"找到了 {len(results)} 个与查询相关的代码片段。请查看下面的代码上下文了解详情。"
                else:
                    # 如果没有LLM处理器，生成一个基本回答
                    answer = f"找到了 {len(results)} 个与查询相关的代码片段。请查看下面的代码上下文了解详情。"
                ...
                answer = f"找到了 {len(results)} 个与查询相关的代码片段，但无法生成详细解释: {str(e)}"
    ...
    def get_qa_bot(self):
        ...
        Returns:
            QARetriever实例
        """
        from mcp_code_indexer.qa_retriever import qa_bot
        return qa_bot(self.config, self.indexer, self)