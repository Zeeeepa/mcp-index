...
class CodeAnalyzer:
    ...
    def _initialize_parser(self, language: str) -> None:
        ...
        try:
            ...
            # 检查是否存在预编译的库
            if (os.path.exists(f"{parser_path}.so") or 
                os.path.exists(f"{parser_path}.dll") or 
                os.path.exists(f"{parser_path}.dylib")):
                
                # 加载语言
                lang_obj = Language(parser_path, language)
                ...
                logger.debug(f"{language} 解析器初始化成功")
            else:
                logger.warning(f"{language} 解析器库文件不存在: {parser_path}.*")
                return
                
        except Exception as e:
            logger.error(f"初始化 {language} 解析器失败: {str(e)}")
            # 不再抛出异常，以允许系统继续运行
            return
        
        # 检查特定语言的初始化是否成功
        if language not in self.parsers:
            logger.error(f"{language}解析器初始化失败，这将导致{language}代码分析功能不可用")