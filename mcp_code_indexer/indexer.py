...
class CodeIndexer:
    ...
    def count_indexed_records(self, project_id: str) -> int:
        """
        计算项目已索引的记录数量
        ...
        try:
            ...
            # 获取所有记录的ID（但不获取实际内容以提高性能）
            records = collection.get(include=[])
            return len(records['ids'])
        except Exception as e:
            logger.error(f"获取索引记录数失败: {str(e)}")
            return 0
    ...
    def _initialize_vector_db(self) -> None:
        ...
        try:
            ...
            logger.info(f"向量数据库初始化完成: {self.vector_db_path}")
        except Exception as e:
            logger.error(f"初始化向量数据库失败: {str(e)}")
            raise

    def _ensure_chroma_connection(self) -> bool:
        """
        确保ChromaDB连接有效
