...
class Config:
    ...
    def initialize_embeddings(self):
        ...
        """
        embedding_model = self.get("indexer.embedding_model")
        use_code_embeddings = self.get("indexer.use_code_embeddings", False)
        
        if use_code_embeddings:
            # 使用代码专用嵌入模型
            try:
                from langchain_huggingface import HuggingFaceEmbeddings
                code_model_name = self.get("indexer.code_embedding_model", "jinaai/jina-embeddings-v2-base-code")
                
                logger.info(f"使用代码专用嵌入模型: {code_model_name}")
                return HuggingFaceEmbeddings(
                    model_name=code_model_name,
                    model_kwargs={"trust_remote_code": True},
                )
            except ImportError:
                logger.warning("无法加载HuggingFaceEmbeddings，将使用默认嵌入模型")
                # 继续使用默认模型
        
        # 使用默认的sentence-transformers模型
        try:
            from sentence_transformers import SentenceTransformer
            logger.info(f"使用默认嵌入模型: {embedding_model}")
