    ...
    def vector_db_status():
        ...
        try:
            ...
            for project in projects:
                ...
                record_count = indexer.count_indexed_records(project_id)
                total_records += record_count
                
                project_stats.append({
                    'project_id': project_id,
                    'name': os.path.basename(project.get('project_path', '')),
                    'record_count': record_count
                })
            
            # 检查连接状态
            connection_valid = indexer._ensure_chroma_connection()
            
            return jsonify({
                'status': 'ok' if connection_valid else 'error',
                'total_projects': len(projects),
                'total_records': total_records,
                'projects': project_stats,
                'db_path': str(indexer.vector_db_path)
            })
        except Exception as e:
